...
namespace Clutchlit.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        public OrdersController(MysqlContext contextSp24)
        {
        ...
        {
            var all_k = _contextSp24.Orders_sp24.Where(m => m.Id_cart != 0);
            ...
            result = (from orders in all_k
                      ...
                      select new A_orders_display { Created = orders.Created, Id_carrier = orders.Id_carrier, Id_address_i = string.Format("{0} {1} <br /> {2} {3} <br/><b>NIP:</b> {4}", invoice.Address1, invoice.Address2, invoice.ZipCode, invoice.City, invoice.Nip), Id_order = orders.Id_order, Current_state = states.Name,
                      Id_address_d = string.Format("{0} {1} {2} {3}", addresses.Address1, addresses.Address2, addresses.ZipCode, addresses.City), Id_cart = orders.Id_cart, Id_customer = string.Format("<b>{2}</b> {0} {1}", customers.FirstName, customers.LastName, customers.Company), Payment = orders.Payment, Reference =orders.Reference, Total_paid = orders.Total_paid, Total_paid_products = orders.Total_paid_products, Total_shipping = orders.Total_shipping,
